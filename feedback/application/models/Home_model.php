...
class Home_model extends CI_Model
{
    ...
    public function getCourses($sid) {
        $this->db->select("t1.*, GROUP_CONCAT(t2.teacher_id) AS teacher_id, GROUP_CONCAT(t2.teacher_status) AS teacher_status");
        $this->db->from("semester_course t1");
        $this->db->join("semester_course_instructor t2","t2.course_id = t1.course_id", "LEFT");
        $this->db->where("t1.student_id", $sid);
        $this->db->where("t1.semester_id='Jun_2015'");
